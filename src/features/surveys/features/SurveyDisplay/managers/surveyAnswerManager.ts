...
export const useSurveyAnswerManager = (
  ...
  const isAnswerValid = (index: number) => {
    ...
    // SECTION and SECTION_BREAKER questions don't require answers
  ...
  const isAnswersValid = () => {
    // Filter out SECTION and SECTION_BREAKER questions for validation
    const questionsRequiringAnswers = formData?.questions?.filter(
      (question) => question.type !== 'SECTION' && question.type !== 'SECTION_BREAKER'
    ) || [];
    ...
    if (
      questionsRequiringAnswers.every(
